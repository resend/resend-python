...
async def main() -> None:
    received_email: resend.ReceivedEmail = await resend.Emails.Receiving.get_async(
        email_id=email_id
    )
    ...
    paginated_emails: EmailsReceiving.ListResponse = (
        await resend.Emails.Receiving.list_async(params=list_params)
    )
    ...
    if paginated_emails["data"] and paginated_emails["has_more"]:
        ...
        next_page: EmailsReceiving.ListResponse = (
            await resend.Emails.Receiving.list_async(params=next_page_params)
        )
    ...
    all_attachments: EmailsReceiving.Attachments.ListResponse = (
        await resend.Emails.Receiving.Attachments.list_async(email_id=email_id)
    )
